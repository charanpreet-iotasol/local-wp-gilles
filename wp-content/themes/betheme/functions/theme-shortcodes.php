...
if (! function_exists('sc_product_upsells')) {
	function sc_product_upsells($attr, $product = false)
	{
		...
		$output .= '<h3 class="title">'.__( 'You may also like&hellip;', 'woocommerce' ).'</h3>';
...
if (! function_exists('sc_product_additional_information')) {
	function sc_product_additional_information($attr, $product = false)
	{
		...
		ob_start();
		if($attr['title'] == 1) echo '<h3>'. esc_html( __( 'Additional information', 'woocommerce' ) ). '</h3>';
...
if (! function_exists('sc_product_reviews')) {
	function sc_product_reviews($attr, $product = false)
	{
		$output = '';
		if( !$product ){
			$output = '<div class="mfn-widget-placeholder mfn-wp-product-reviews"><img class="item-preview-image" src="'.get_theme_file_uri('/muffin-options/svg/placeholders/product_rating.svg').'"></div>';
		}else{
			setup_postdata($product->get_id());
...
if (! function_exists('sc_product_breadcrumbs')) {
	function sc_product_breadcrumbs($attr, $product = false)
	{
		...
		if( is_woocommerce() ){
			...
			// for builder only
			$output = '<nav class="woocommerce-breadcrumb mfn-woocommerce-breadcrumb"><a href="#" class="mfn-woo-breadcrumb-home">Home<span class="mfn-woo-breadcrump-delimiter">' . $args['delimiter'] . '</span></a> <a href="#">'.__('Product Category', 'woocommerce').'</a><span class="mfn-woo-breadcrump-delimiter">' . $args['delimiter'] .'</span>'. get_the_title($product->get_id()) .'</nav>';
		}
...
if (! function_exists('sc_shop_products')) {
	function sc_shop_products($attr, $sample = false){
		...
		$output .= '<div class="products_wrapper mfn-woo-products">';
 		// start loop
		...
		if(is_woocommerce() && have_posts()):
		$output  .= '<ul class="products '.implode(' ', $classes).'">';
			while ( have_posts() ) {
				the_post();
				global $product;
				if($layout == 'masonry') $classes = array( 'isotope-item' );
				if ( empty( $product ) || ! $product->is_visible() ) return; // visibility
		...
		elseif( $sample ):
			...
			$output .= '<div class="alert alert_info">';
				$output .= '<div class="alert_icon"><svg viewBox="0 0 28 28"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><circle class="path" cx="14" cy="14" r="12"/><path class="path" d="M11.2,9.12a3.4,3.4,0,0,1,3-1.69,2.84,2.84,0,0,1,3,2.76,3.16,3.16,0,0,1-.84,2.23c-.63.74-1.58,1.18-2.19,1.88a1,1,0,0,0-.26.64v2.32"/><circle class="path" cx="14" cy="20" r="0.33"/></g></svg></div>';
...
if (! function_exists('sc_shop_categories')) {
	function sc_shop_categories($attr)
	{
    	...
    	if( isset($attr['category']) && !empty($attr['category']) ){
    		$parent = get_term_by('slug', $attr['category'], 'product_cat')->term_id;
    	}

		$cats = get_terms(
			'product_cat',
			array(
				'hide_empty' => isset($attr['empty']) ? $attr['empty'] : 0,
				'child_of' => $parent,
			)
		);
...
if (! function_exists('sc_blog_news')) {
	function sc_blog_news($attr, $content = null)
	{
		...
		$output = '<div class="Latest_news '. esc_attr($style) .'">';
			...
			if ($link) {
				$output .= '<a class="button has-icon" href="'. esc_url($link) .'"><span class="button_icon"><i class="icon-layout"></i></span><span class="button_label">'. esc_html($link_title) .'</span></a>';
			}
...
if (! function_exists('sc_popup')) {
	function sc_popup($attr, $content = null)
	{
		...
		if ($button) {
			$output .= '<a href="#'. esc_attr($uid) .'" rel="lightbox" data-type="inline" class="popup-link button"><span class="button_label">'. wp_kses($title, mfn_allowed_html()) .'</span></a>';
		} else {
			$output .= '<a href="#'. esc_attr($uid) .'" rel="lightbox" data-type="inline" class="popup-link">'. wp_kses($title, mfn_allowed_html()) .'</a>';
		}
				...
				$output .= '<div class="image_frame image_item scale-with-grid'. esc_attr($class_div) .'" '. $style_escaped .'>';
					...
					$output .= '<div class="image_wrapper">';
						...
						$output .= '<div class="image_links '. esc_attr($double_link) .'">';
							if ($link_image) {
								$output .= '<a href="'. esc_url($link_image) .'" class="zoom" rel="prettyphoto"><svg viewBox="0 0 26 26"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle cx="11.35" cy="11.35" r="6" class="path"/><line x1="15.59" y1="15.59" x2="20.65" y2="20.65" class="path"/></svg></a>';
							}
							if ($link) {
								// This variable has been safely escaped above in this function
								$output .= '<a href="'. esc_url($link) .'" class="link" '. $target_escaped .'><svg viewBox="0 0 26 26"><defs><style>.path{fill:none;stroke:#333;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><path d="M10.17,8.76l2.12-2.12a5,5,0,0,1,7.07,0h0a5,5,0,0,1,0,7.07l-2.12,2.12" class="path"></path><path d="M15.83,17.24l-2.12,2.12a5,5,0,0,1-7.07,0h0a5,5,0,0,1,0-7.07l2.12-2.12" class="path"></path><line x1="10.17" y1="15.83" x2="15.83" y2="10.17" class="path"></line></g></svg></a>';
							}
						$output .= '</div>';
...
if (! function_exists('sc_hover_color')) {
	function sc_hover_color($attr, $content = null)
	{
		...
		if( is_numeric($border_radius) ){
			$border_radius_outer = ($border_radius +1) .'px';
			$border_radius .= 'px';
		} else {
			$border_radius_outer = $border_radius;
		}
		...
		$output = '<div class="hover_color align_'. esc_attr($align) .'" style="background-color:'. esc_attr($background_hover) .';border-color:'. esc_attr($border_hover) .';border-radius:'. esc_attr($border_radius_outer) .';'. esc_attr($style) .'" ontouchstart="this.classList.toggle(\'hover\');">';
			$output .= '<div class="hover_color_bg" style="background-color:'. esc_attr($background) .';border-color:'. esc_attr($border) .';border-width:'. esc_attr($border_width) .';border-radius:'. esc_attr($border_radius) .';">';
		...
		$output = '<a class="mfn-link mfn-link-'. intval($style, 10) .' '. esc_attr($class) .'" href="'. esc_url($link) .'" style="'. $css .'" data-hover="'. esc_html($title) .'" ontouchstart="this.classList.toggle(\'hover\');" '. $target_escaped .' '. $download_escaped .'>';
			...
			if( 9 == $style ){
				if( $icon ){
					...
					$output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 14"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><g><line class="path" x1="25.23" y1="7" x2="3.23" y2="7"/><polyline class="path" points="20.47 3.5 25.23 7 20.47 10.5"/></g></svg>';
				}
			}
			...
			if ($animate) {
				...
				$output .= '<div class="desc_wrapper">';
					...
					if ($email || $phone || $facebook || $twitter || $linkedin) {
						$output .= '<div class="links">';
							if ($email) {
								$output .= '<a href="mailto:'. esc_attr($email) .'" class="icon_bar icon_bar_small mail"><span class="t"><i class="icon-mail"></i></span><span class="b"><i class="icon-mail"></i></span></a>';
							}
							if ($facebook) {
								$output .= '<a target="_blank" href="'. esc_url($facebook) .'" class="icon_bar icon_bar_small facebook"><span class="t"><i class="icon-facebook"></i></span><span class="b"><i class="icon-facebook"></i></span></a>';
							}
							if ($twitter) {
								$output .= '<a target="_blank" href="'. esc_url($twitter) .'" class="icon_bar icon_bar_small twitter"><span class="t"><i class="icon-twitter"></i></span><span class="b"><i class="icon-twitter"></i></span></a>';
							}
							if ($linkedin) {
								$output .= '<a target="_blank" href="'. esc_url($linkedin) .'" class="icon_bar icon_bar_small linkedin"><span class="t"><i class="icon-linkedin"></i></span><span class="b"><i class="icon-linkedin"></i></span></a>';
							}
							if ($vcard) {
								$output .= '<a href="'. esc_url($vcard) .'" class="icon_bar icon_bar_small vcard"><span class="t"><i class="icon-vcard"></i></span><span class="b"><i class="icon-vcard"></i></span></a>';
							}
						$output .= '</div>';
					}
	...
	function sc_map($attr, $content = null)
	{
		...
		$inline_script = 'function mfn_google_maps_'. esc_attr($uid) .'(){';
		  ...
		  if ( ! empty( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && strtolower( $_SERVER['HTTP_X_REQUESTED_WITH'] ) == 'xmlhttprequest' ){
		  	$inline_script .= 'var mfn_iframe = document.getElementById("mfn-vb-ifr");';
		  	$inline_script .= 'var map = new google.maps.Map(mfn_iframe.contentWindow.document.getElementById("google-map-area-'. esc_attr($uid) .'"), myOptions);';
		  } else {
		...
		$output  = '<div class="content_video '. esc_attr($class) .'">';
			...
			if ($video) {
				...
				$output .= '<div class="section_video">';
					...
					// This variable has been safely escaped above in this function
					$output .= '<video poster="'. esc_url($poster) .'" '. $html5_escaped .' width="'. esc_attr($width) .'" height="'. esc_attr($height) .'" style="max-width:100%;">';
	...
	function sc_livesearch($attr, $content = null)
	{
		...
		$output = '<div class="mfn-live-search-wrapper" data-char="'.$atts['min_characters'].'" data-posts="'.$atts['load_posts'].'" data-featured="'.$atts['featured_image'].'">';
			...
			$output .= '<form method="get" id="searchform" action="'. esc_url( home_url('/') ) .'">';
				$output .= '<svg class="icon_search" width="26" viewBox="0 0 26 26"><defs><style>.path{fill:none;stroke:#000;stroke-miterlimit:10;stroke-width:1.5px;}</style></defs><circle class="path" cx="11.35" cy="11.35" r="6"></circle><line class="path" x1="15.59" y1="15.59" x2="20.65" y2="20.65"></line></svg>';
...
if (! function_exists('sc_table_of_contents')) {
	function sc_table_of_contents($attr, $content = null)
	{
		...
		//get content from muffin builder
		...
		//The purpose of it, is to get the column content of Muffin Builder
		if( is_array($mfn_sections) ){
			...
			//find columns content only!
			foreach($mfn_items as $key => $value){
				if( is_array($value) ){
					foreach($value as $in_key => $in_val){
						if ( isset($in_val['type']) && $in_val['type'] === 'column') {
		...
		// output -----
			...
			// This variable has been safely escaped above in this function
			$output = '';
			$output .= '<div class="table_of_content" data-tags="'.$tags_anchors.'">';

				if ($title) {
					$output .= '<'.strtolower($title_tag).'>'. wp_kses($title, mfn_allowed_html()) .'</'.strtolower($title_tag).'>';
				}
						...
						foreach($mfn_headings as $key => $value){
							...
							switch(true){
								case $previous_heading_level === 0: //init
									$output .= '<li data-depth="'.$nested_depth.'" >'. $icon .'<a class="scroll" href="#toc_'.$key.'">'. wp_strip_all_tags(do_shortcode($value)) .'</a></li>';
								...
								case $highest_heading === $matches[0] && $nested_depth > 1: //root heading reached, reset
									...
									$output .= '<li>'. $icon .'<a class="scroll" href="#toc_'.$key.'">'. wp_strip_all_tags(do_shortcode($value)) .'</a></li>';
